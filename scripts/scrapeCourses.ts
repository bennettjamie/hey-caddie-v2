 ...
 */
export function fetchCoursePage(courseId: string, urlOverride?: string): Promise<string> {
    return new Promise((resolve, reject) => {
        const url = urlOverride || `https://www.dgcoursereview.com/course.php?id=${courseId}`;
            ...
            if (res.statusCode === 301 || res.statusCode === 302) {
                if (res.headers.location) {
                    ...
                    if (newUrl.startsWith('/')) {
                        ...
                        newUrl = `https://www.dgcoursereview.com${newUrl}`;
                    }
                    fetchCoursePage(courseId, newUrl).then(resolve).catch(reject);
                    return;
                }
            }
 ...
 */
export function parseCourseFromHTML(html: string, courseId: string): ScrapedCourse | null {
    try {
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1].endsWith('scrapeCourses.ts')) {
